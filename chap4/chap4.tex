...
\end{figure}
\subsection{実験結果}
Fig.に障害物を右に回避した時の比較動合成画像を，Fig.に障害物を左に回避した時の比較動合成画像を示す．Fig.およびFig.より知識を選択しながら障害物を回避することに成功した．また，ステップ数においての知識番号選択のグラフをFig.\ref{migikaihi}とFig.\ref{hidarikaihi}に示す．
\begin{figure}[h]
	\centering
...
本章では，提案手法に基づいた実験の詳細について述べた．\ref{4 RLsim}節では，強化学習実験の目的，条件，および結果について述べた．\ref{4 SAP-net}節では，SAP-netを実装したシミュレーション実験の目的，条件，および結果について述べた．\ref{4 zikki}節では，移動軌跡を転移させることができた．本実験環境において，強化学習を用いた障害物回避の有用性，SAP-netを用いて複数の障害物の環境下を含めた障害物回避の有用性，実機実装を用いての障害物回避が可能であることを示した．
\clearpage
\newpage
